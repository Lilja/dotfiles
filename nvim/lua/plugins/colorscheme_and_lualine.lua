...
--- @param qoute string[]
local function word_wrap_quote(qoute)
	-- Wraps the quote at 80 characters.
	local output = {}
	local wrap_len = 70

	for _, sentence in ipairs(qoute) do
		if #sentence > wrap_len then
			-- Iterate over every word in the sentence and try to wrap it at 80 characters.
			local word_wrapped_sentence = {}
			local current_line = ""

			for word in sentence:gmatch("(%S+)") do
				if #current_line + #word + 1 < wrap_len then
					current_line = current_line .. " " .. word
				else
					table.insert(word_wrapped_sentence, current_line)
					current_line = word
				end
			end
			table.insert(word_wrapped_sentence, current_line)
			for _, line in ipairs(word_wrapped_sentence) do
				table.insert(output, line)
			end
		else
			table.insert(output, sentence)
		end
	end
	return output
end
